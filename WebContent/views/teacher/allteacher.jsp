...
<head>
	...
	$(function() {	
	    ...
	    $('#dataList').datagrid({ 
	        ...
	        fit: true,//自动大小 
	        method: "post",
	        url:"TeacherServlet?method=AllTeacherList&t="+new Date().getTime(),
	        idField:'id', 
	    ...
	    $("#addDialog").dialog({
	  	...
	  	$("#editDialog").dialog({
			...
			onBeforeOpen: function(){
				...
				$("#edit_photo").attr("src", "PhotoServlet?method=getPhoto&tno="+selectRow.tno);
				$("#set-photo-tno").val(selectRow.tno);
			}
	    });
		//格式化增加生日下拉框
	  	$('#add_rztime').datebox({
	...
	function uploadPhoto(){
		...
		$("#uploadForm").attr('action',action+'&tno='+$("#set-photo-tno").val());
		$("#uploadForm").submit();
		setTimeout(function(){
			$.messager.alert("消息提醒","已更改(错误不更改)","info");
			$("#edit_photo").attr("src", "PhotoServlet?method=getPhoto&sno="+$("#set-photo-tno").val());
		}, 1500)
	}
	</script>
</head>
		...
		<div id="editDialog" style="padding: 10px">  
			<div style="float: right; margin: 20px 20px 0 0; width: 200px; border: 1px solid #EBF3FF">
	    		...
	    		<form id="uploadForm" method="post" enctype="multipart/form-data" action="PhotoServlet?method=SetPhoto" target="photo_target">
	    			<input type="hidden" name="tno" id="set-photo-tno">
